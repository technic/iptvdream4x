...
class OTTProvider(AbstractStream):
	NAME = "MoiDomTV"
	site = "http://iptv.moi-dom.tv/api/json"
	...
	def parseAccount(self, account):
		expire = datetime.fromtimestamp(int(account['packet_expire']))
	...
	def getChannelsEpg(self, cids):
		data = self.getJsonData(self.site + "/epg_next?", {"cids": ",".join(map(str, cids))})
		# data = self.getJsonData(self.site + "/epg_current?", {"cids": ",".join(map(str, cids))})
		for e in data['epg']:
			yield int(e['cid']), EPG(int(e['epg_start']), int(e['epg_end']), e['epg_progname'].encode('utf-8'))

	def getCurrentEpg(self, cid):
		data = self.getJsonData(self.site + "/epg_next?", {"cid": cid})
		epg = data['epg']
		for i, e in enumerate(epg[:-1]):
			yield EPG(
				int(epg[i]['ts']), int(epg[i+1]['ts']),
				e['progname'].encode('utf-8'))

	def getDayEpg(self, cid, date):
		data = self.getJsonData(self.site + "/epg?", {'cid': cid, 'day': date.strftime("%d%m%y")})
		for e in data['epg']:
			yield EPG(
				int(e['ut_start']), int(e['ut_stop']),
				e['progname'].encode('utf-8'))
